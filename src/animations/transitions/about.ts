...
const setup = ({
  ...
  tlExperience?: gsap.core.Timeline;
  contentExperience?: HTMLDivElement;
}) => {
...
const setupSectionsAnimation = ({
  ...
  sectionsMm = createMatchMedia((_context, { isLandscape }) => {
    ...
    if (isLandscape) {
      // Spacing details: Details at 0, Description at 0.3, Services at 0.6, Experience at 0.6
      const DETAILS_DELAY = 0;
      const DESCRIPTION_DELAY = 0.3;
      const SERVICES_DELAY = 0.6;
      const EXPERIENCE_DELAY = 0.6;
      ...
      // Experience animation (landscape right column bottom)
      if (contentExperience) {
        tl.fromTo(contentExperience, { opacity: 0 }, { opacity: 1, duration: 0.15, ease: "power1.out" }, EXPERIENCE_DELAY);
        tl.add(() => {
          tlExperience?.play();
        }, EXPERIENCE_DELAY);
      }
    } else {
      // Mobile: description, services, then experience chronologically
      const DESCRIPTION_DELAY = 0;
      const SERVICES_DELAY = 0.4;
      const EXPERIENCE_DELAY = 0.8;

      // Description animation
      tl.fromTo(
        contentDescription,
        { opacity: 0, y: "10vh" },
        { opacity: 1, y: "0vh", duration: 0.12, ease: "power1.out" },
        DESCRIPTION_DELAY,
      );
      tl.to(contentDescription, { opacity: 0, y: "-10vh", duration: 0.12, ease: "power1.out" }, SERVICES_DELAY - 0.05);
      tl.add(() => {
        tlDescription?.play();
      }, DESCRIPTION_DELAY);
      ...
      tl.to(contentServices, { opacity: 0, y: "-10vh", duration: 0.12, ease: "power1.out" }, EXPERIENCE_DELAY - 0.05);
      tl.add(() => {
        tlServices?.play();
      }, SERVICES_DELAY);
